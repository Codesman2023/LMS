...
export default async function NotesPage() {
  try {
    ...
    console.error("Failed to load notes page:", error.message);

    return (
      <div className="container mx-auto p-6">
        <h1 className="text-3xl font-bold mb-4 text-center">Notes</h1>
        <p className="mx-auto max-w-2xl rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-center text-red-700">
          Notes are temporarily unavailable. Check the `MONGODB_URI` value in
          Vercel and confirm your MongoDB Atlas network access allows Vercel to
          connect.
        </p>
      </div>
    );
